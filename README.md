...
Finally, you will need GitHub credentials to view the application. You will not be able to update todo items without changing the logic in the `User` class (which sets the `role` attribute based on GitHub id). 

1. `CLIENT_ID` - Identifier for the application
1. `CLIENT_SECRET` - Client secret generated by GitHub
